...
export default function TextInputS2(props) {
    return (
        <div style={{alignSelf: 'stretch', height: 'auto', flexDirection: 'column', justifyContent: 'center', alignItems: 'flex-start', gap: 5, display: 'flex', width:props.width}}>
            ...
            <textarea placeholder={props.description} spellCheck='false' className="textarea" id={props.id} style={{alignSelf: 'stretch', height: 'auto', background: 'white', borderRadius: 53, padding: 22, fontFamily: props.font, fontSize: 20, border: '2px solid black'}} type={props.type} onChange={props.onChange} rows={4} cols={1}></textarea>
        </div>
    );
}