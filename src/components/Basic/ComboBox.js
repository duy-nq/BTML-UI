    ...
    return (
        <>
            <div style={{width: 400, flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'flex-start', gap: 8, display: 'inline-flex'}}>
                ...
                <select 
                    id="ac"
                    style={{border:'none', alignSelf: 'stretch', height: 62, background: 'rgba(147, 168, 244, 0.17)', borderRadius: 25, padding: '0px 20px 0px 20px', fontSize: 24, fontFamily: 'Inria Sans',}} 
                    onChange={props.func}
                >
                    <option value="">Choose an option</option>
                    {props.list.map((item) => (
                        <option key={item.id} value={item[props.valueProp]}>
                            {item[props.nameProp]}
                        </option>
                    ))}
                </select>
